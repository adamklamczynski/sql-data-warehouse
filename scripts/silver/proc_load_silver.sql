...
BEGIN 
	...
	BEGIN TRY
		...
		SELECT 
			...
			END AS sls_due_dt,
			CASE 
				WHEN sls_sales IS NULL OR sls_sales <=0 OR sls_sales != sls_price * ABS(sls_quantity) THEN sls_quantity * ABS(sls_price)
				ELSE sls_sales
			END AS sls_sales,
			sls_quantity,

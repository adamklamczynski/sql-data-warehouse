...
Script Purpose:
  ...
  to produce a clean, enriched and businnes-ready dataset.
...
SELECT
	...
	CASE 
		WHEN ci.cst_gender != 'N/A' THEN ci.cst_gender --CRM is primary sourse
...
ON cp.cat_id=ep.id
WHERE cp.prd_end_dt IS NULL; --Filter active records only
GO
...
LEFT JOIN gold.dim_customers cg
ON sd.sls_cust_id = cg.customer_id;
GO
